...
export const useFormSubmit = async (props: FormSubmit) : Promise<void> => {
    const { url, dataToSend, router, validationZod, setErrors, message, redirectURL } = props;
    ...
    if(!result.success) {
        ...
        if((dataToSend.keywords as string[]).length < 3) setErrors((prevErrors) => ({ ...prevErrors, keywords: "Оберіть мінімум 3 ключові слова" }));
